...
{
    public class TbAccessPermissionManager : ITbAccessPermissionManager
    {
        private readonly ITbAccessPermissionDataAccess DataAccess = null;
        public TbAccessPermissionManager(ITbAccessPermissionDataAccess dataAccess)
        {
        ...
        public APIResponse Insert(tbAccessPermissionModel model)
        {
        ...
        public APIResponse Update(Guid GUIDAccessPermission, tbAccessPermissionModel model)
        {
            var result = DataAccess.Update(GUIDAccessPermission, model);
            if (result)
            {
                return new APIResponse(ResponseCode.SUCCESS, "Record Inserted", result);
            }
        ...
        public APIResponse HardDelete(Guid GUIDAccessPermission)
        {
            var result = DataAccess.HardDelete(GUIDAccessPermission);
            if (result)
            {
                return new APIResponse(ResponseCode.SUCCESS, "Record Inserted", result);
            }
