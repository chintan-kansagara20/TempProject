...
{
    public class TbWarehouseManager : ITbWarehouseManager
    {
        private readonly ITbWarehouseDataAccess DataAccess = null;
        public TbWarehouseManager(ITbWarehouseDataAccess dataAccess)
        {
        ...
        public APIResponse Insert(tbWarehouseModel model)
        {
        ...
        public APIResponse Update(Guid GUIDWarehouse, tbWarehouseModel model)
        {
            var result = DataAccess.Update(GUIDWarehouse, model);
            if (result)
            {
                return new APIResponse(ResponseCode.SUCCESS, "Record Inserted", result);
            }
        ...
        public APIResponse HardDelete(Guid GUIDWarehouse)
        {
            var result = DataAccess.HardDelete(GUIDWarehouse);
            if (result)
            {
                return new APIResponse(ResponseCode.SUCCESS, "Record Inserted", result);
            }
