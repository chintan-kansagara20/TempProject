...
{
    public interface ITbCustomerShipToInfoManager
    {
        APIResponse Get(int page, int itemsPerPage, List<OrderByModel> orderBy, List<AdvanceFilterByModel> filtersList);
        APIResponse Insert(tbCustomerShipToInfoModel model);
        APIResponse Update(Guid GUIDLocation, tbCustomerShipToInfoModel model);
